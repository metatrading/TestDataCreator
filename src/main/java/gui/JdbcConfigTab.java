...
public class JdbcConfigTab extends Tab {
    ...
    public void init() {
        ...
        settingPane.add(
                newTextField(dao.getUrl(),
                        "jdbc:{subprotocol}://{DBHost}\\{InstanceName};databaseName={DBName}"), 1,
                0);
        ...
        saveButton.setOnAction(b -> {
            ...
            try {
                ...
                try (var v = Files.find(checkPath,
                        Integer.MAX_VALUE, (p, attr) -> !attr.isDirectory() &&
                                p.toFile().getName().startsWith(dao.getSettingFilePrefix()))) {
                    v.forEach(t -> {
                        try {
                            Files.delete(t);
                        } catch (IOException e) {
            ...
            // 保存
            try (FileWriter out = new FileWriter(Objects.requireNonNull(file))) {
